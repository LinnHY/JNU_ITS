<template>
    <div class="home">
        ...
        <main>
            <!-- H5新标签 定义文档的某个区域 ComponentList为左侧组件列表-->
            <section class="left">
                <ComponentList />
                <RealTimeComponentList />
            </section>
            <!-- 中间画布 handleDrop中的dataTransfer.getData()用于接收传输过来的drag数据-->
...
export default {
    ...
    methods: {
        ...
        handleDrop(e) {
            ...
            if (index) {
                const component = deepCopy(componentList[index]); // 根据索引找到对应的组件数据
                console.log("component.icon = ", component.icon);
                ...
                this.$store.commit("addComponent", { component }); // 在画布上渲染这次拖出的组件
                this.$store.commit("recordSnapshot");
            }
        },
        ...
        deselectCurComponent(e) {
            ...
            // 0 左击 1 滚轮 2 右击
            if (e.button != 2) {
                this.$store.commit("hideContextMenu"); // 若鼠标按下时不是右键，则不显示右击菜单
            }
        },
    },
};
